...
func TestRunCostWritWithTokenData(t *testing.T) {
	...
	if _, err := s.WriteTokenUsage(h1, "claude-sonnet-4-6", 100000, 40000, 80000, 10000, 0, nil, nil, "", ""); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := s.WriteTokenUsage(h2, "claude-sonnet-4-6", 24532, 5201, 9100, 2340, 0, nil, nil, "", ""); err != nil {
		t.Fatal(err)
	}
	if _, err := s.WriteTokenUsage(h2, "claude-haiku-3-5", 5000, 2000, 1000, 500, 0, nil, nil, "", ""); err != nil {
		t.Fatal(err)
	}
...
func TestRunCostWritWithCostUSD(t *testing.T) {
	...
	cost := 1.23
	if _, err := s.WriteTokenUsage(h1, "claude-sonnet-4-6", 100000, 40000, 80000, 10000, 0, &cost, nil, "", ""); err != nil {
		t.Fatal(err)
	}
...
func TestRunCostWritWithSinceFilter(t *testing.T) {
	...
	if _, err := s.WriteTokenUsage(h1, "claude-sonnet-4-6", 50000, 20000, 30000, 5000, 0, nil, nil, "", ""); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := s.WriteTokenUsage(h2, "claude-sonnet-4-6", 10000, 5000, 3000, 1000, 0, nil, nil, "", ""); err != nil {
		t.Fatal(err)
	}
...
func TestRunCostWritJSON(t *testing.T) {
	...
	if _, err := s.WriteTokenUsage(h1, "claude-sonnet-4-6", 1000, 500, 200, 100, 0, nil, nil, "", ""); err != nil {
		t.Fatal(err)
	}
