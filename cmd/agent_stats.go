...
func computeAgentStats(worldStore *store.WorldStore, agentName string) (*AgentStatsReport, error) {
	...
	// 4. Cost estimation from OTLP data.
	if len(tokens) > 0 {
		var total float64
		allHaveCost := true
		for _, t := range tokens {
...
func renderAgentStats(r *AgentStatsReport) {
	...
	if r.EstimatedCost != nil {
		...
		b.WriteString(fmt.Sprintf("  %-18s %s\n", "Estimated cost:", dimStyle.Render("N/A")))
	}
