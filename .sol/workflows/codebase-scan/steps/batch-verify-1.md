# Batch Verify 1 — Data Layer

Verify findings from the **store**, **config-and-setup**, and **session-lifecycle** analysis steps.
...
- `store` — SQLite store layer (internal/store/)
- `config-and-setup` — Configuration, setup, and utilities (internal/config/, setup/, fileutil/, etc.)
- `session-lifecycle` — Session lifecycle (internal/startup/, dispatch/, session/, tether/, adapter/, handoff/)
...
Write to `batch-verify-1.md` in your output directory.
