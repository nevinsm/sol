...
| **Memory** | An envoy's persistent accumulated context. Claude Code auto-memory, loaded from `<envoyDir>/memory/MEMORY.md` at session start via the Claude runtime's `autoMemoryDirectory` setting (Claude-only; the Codex runtime does not support persistent memory). Agent-maintained via the `/memory` REPL command or natural-language save. Lives outside the worktree so it survives worktree rebuilds. | *(was: Brief, retired)* |
...
| **SessionSave** | Best-effort "prompt agent to save state, wait for stability" primitive. Used before destructive session operations (handoff) to reduce context loss. Package: `internal/sessionsave/`. | *(new)* |
