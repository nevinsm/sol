...
`main` history is forge-authored: forge squashes each writ's branch to a single commit with message `{writ title} (writ-id)` — a sentence-case summary plus the writ ID in parens, e.g. `Skip credential symlink when a credential env var is configured (sol-3e0294f18e8267cc)`. This is generated by `internal/forge/injection.go`, not hand-written by agents.

Agent commits on their own branch, before forge squashes them, have no mandated format — just commit early and often with clear, descriptive messages (see `internal/guidelines/defaults/default.md`).
