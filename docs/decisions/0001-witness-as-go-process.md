# ADR-0001: Witness as Go Process with Targeted AI Call-outs
...
The target architecture (Section 3.8) originally specified the witness as
an "AI Agent, Per-Rig" — implying a full Claude Code session running in
tmux, similar to a polecat. The witness's job is to patrol polecats,
...
- Hook state check → deterministic (file exists or not)
- Stalled detection → deterministic (session dead + hook set)
- Zombie detection → deterministic (session alive + idle + no hook)
...
rig for a patrol loop that is 95% deterministic wastes API credits and
adds an unnecessary failure mode (the AI session itself can get stuck,
context-compacted, or confused).

## Decision

Implement the witness as a Go process that uses targeted `claude -p`
...
(tmux output hash unchanged between consecutive patrols), the witness
...
The assessment command is configurable (`AssessCommand` in witness
...
- No risk of the witness itself getting stuck, context-compacted, or
  confused — it's a Go binary
- Easier to test — mock the assessment function, test patrol logic in
  isolation
- Supervisor can restart the witness instantly (Go binary, not a
  Claude session that needs priming)

**Tradeoffs:**
- Less flexible than a full AI agent — the witness can't reason about
...
- The Deacon (Loop 5) will provide AI-agent-level judgment for
  situations the witness can't handle
