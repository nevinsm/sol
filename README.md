...
You create **work items** describing what needs to be done. When you **cast** a work item, sol picks an idle agent (or creates one), sets up an isolated git worktree, writes the assignment to a **tether** file, and starts a Claude session. The agent reads its tether and gets to work.

When an agent finishes, it calls `sol resolve` — this pushes its branch, updates the work item status, and clears the tether. The **forge** picks up completed branches and merges them through quality gates into the target branch.
...
Everything is inspectable. Work items live in SQLite — query them with `sqlite3`. Tethers are plain files — read them with `cat`. Sessions are tmux — attach with `sol session attach`.
...
| **Cast** | Dispatch a work item: create worktree, write tether, start session. |
...
| **Caravan** | A batch of related work items dispatched as a group. |
...
- **World DB** (`$SOL_HOME/.store/{world}.db`) — work items, labels, merge requests, dependencies
