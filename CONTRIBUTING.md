...
`main` history is forge-authored, not hand-written: forge squashes each writ's branch to a single commit with message `{writ title} (writ-id)` — a sentence-case summary followed by the writ ID in parens, e.g.:

```
Skip credential symlink when a credential env var is configured (sol-3e0294f18e8267cc)
```

This is generated by `internal/forge/injection.go`; don't hand-author `main` commits in another format.

Commits on your own branch, before forge squashes them, have no mandated format — just commit early and often with clear, descriptive messages.
