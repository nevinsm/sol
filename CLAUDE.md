...
- **Worktree excludes**: Sol-managed local files (`.claude/settings.local.json`, `.claude/CLAUDE.local.md`, `.claude/sol-cli-reference.md`) and sol-specific directories (`.brief/`, `.workflow/`) are excluded from git via `.git/info/exclude` in the managed repo (`setup.InstallExcludes`). The shared `.claude/` contents (`settings.json`, `CLAUDE.md`, `agents/`, `rules/`) are NOT excluded — they belong to the project's version control. Agent persona files are written to `.claude/CLAUDE.local.md` (the local variant) so the project's shared instructions are preserved. If you add a new sol-managed path that gets written inside worktrees, add it to the exclude list in `internal/setup/setup.go`.
