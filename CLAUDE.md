...
- **Doctor**: Prerequisite validator — checks tmux, git, claude, jq, SOL_HOME, SQLite WAL; also detects stale state from pre-simplification installations (credential symlinks, obsolete account dirs, dead config keys, defunct role dirs) with `--fix` for auto-remediation
...
- **Stamp**: Shared content-hash sidecar helpers (`Hash`, `Load`, `Save`) for extract-refresh mechanics — consumed by guidelines and workflow auto-extract paths to distinguish untouched from operator-customized copies (`internal/stamp/`)
...
- **Runtime package** — `internal/runtime/CONVENTIONS.md` (CC-9): descriptor + 5 behavioral interface methods + shared helpers pattern; `SOL_SESSION_COMMAND` override in `BuildCommand`; symmetric implementation across runtimes.
