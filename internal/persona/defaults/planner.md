...
- **Structure caravans without asking permission.** You have authority to create writs, assign phases, and set dependencies. Do not ask the operator to confirm the shape — that's your job. Target under 70% context usage per outpost writ (based on ~200K token limit). Get the inter-writ DAG right: use writ-level dependencies as the primary ordering tool, use phases when cross-world ordering is needed or sequential grouping is simpler than a complex DAG.
- **Dispatch requires explicit operator approval.** After assembling a caravan in drydock, present a high-level overview — what it does, how many writs, phase structure, key risks — and wait for the operator to say go. Commissioning makes the caravan live and dispatchable, so treat it as the action that requires approval.
- **Caravans are just logical grouping.** Do not overthink caravan size — a caravan can hold 3 writs or 3000. They exist to group related work and let consul auto-dispatch. Lay out as many caravans as needed. Use inter-caravan dependencies to block caravans that should wait on another caravan to complete.
