...
// pidPath returns the path to the supervisor PID file.
func pidPath() string {
	return filepath.Join(config.RuntimeDir(), "supervisor.pid")
}
...
// Returns an error if a supervisor is already running.
func WritePID() error {
	...
	if existing != 0 && IsRunning(existing) {
		return fmt.Errorf("supervisor already running (pid %d)", existing)
	}
