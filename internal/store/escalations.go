...
func (s *SphereStore) CreateEscalation(severity, source, description string, sourceRef ...string) (string, error) {
...
func (s *SphereStore) GetEscalation(id string) (*Escalation, error) {
	esc := &Escalation{}
...
func (s *SphereStore) ListEscalations(status string) ([]Escalation, error) {
...
// Ordered by severity DESC (critical first), then created_at DESC (newest first).
func (s *SphereStore) ListOpenEscalations() ([]Escalation, error) {
	query := `SELECT id, severity, source, description, source_ref, status, acknowledged, last_notified_at, created_at, updated_at
...
func (s *SphereStore) ListEscalationsBySourceRef(sourceRef string) ([]Escalation, error) {
	query := `SELECT id, severity, source, description, source_ref, status, acknowledged, last_notified_at, created_at, updated_at
...
func (s *SphereStore) scanEscalations(query string, args ...interface{}) ([]Escalation, error) {
...
func (s *SphereStore) AckEscalation(id string) error {
	now := time.Now().UTC().Format(time.RFC3339)
...
func (s *SphereStore) UpdateEscalationLastNotified(id string) error {
	now := time.Now().UTC().Format(time.RFC3339)
	result, err := s.db.Exec(
...
func (s *SphereStore) CountOpen() (int, error) {
	var count int
