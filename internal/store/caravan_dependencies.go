...
func (ss *SphereStore) AddCaravanDependency(fromID, toID string) error {
	if fromID == toID {
		return fmt.Errorf("caravan %q cannot depend on itself", fromID)
	}

	// Verify both caravans exist.
	if _, err := ss.GetCaravan(fromID); err != nil {
		return fmt.Errorf("caravan %q not found", fromID)
	}
	if _, err := ss.GetCaravan(toID); err != nil {
		return fmt.Errorf("caravan %q not found", toID)
	}

	// Check for cycles.
	cycle, err := detectCycle(ss.GetCaravanDependencies, fromID, toID)
	if err != nil {
...
func (ss *SphereStore) RemoveCaravanDependency(fromID, toID string) error {
	_, err := ss.db.Exec(
...
func (ss *SphereStore) GetCaravanDependencies(caravanID string) ([]string, error) {
...
func (ss *SphereStore) GetCaravanDependents(caravanID string) ([]string, error) {
	rows, err := ss.db.Query(
...
func (ss *SphereStore) AreCaravanDependenciesSatisfied(caravanID string) (bool, error) {
	var unsatisfied int
	err := ss.db.QueryRow(`
...
func (ss *SphereStore) UnsatisfiedCaravanDependencies(caravanID string) ([]string, error) {
	rows, err := ss.db.Query(`
...
func (ss *SphereStore) IsWritBlockedByCaravanDeps(writID string) (bool, []string, error) {
	...
	for _, carID := range caravanIDs {
		satisfied, err := ss.AreCaravanDependenciesSatisfied(carID)
		if err != nil {
			return false, nil, err
		}
		if !satisfied {
			unsatisfied, err := ss.UnsatisfiedCaravanDependencies(carID)
			if err != nil {
				return false, nil, err
			}
...
func (ss *SphereStore) DeleteCaravanDependencies(caravanID string) error {
	_, err := ss.db.Exec(
...
func (ss *SphereStore) IsWritBlockedByCaravan(writID, world string,
	worldOpener func(world string) (*Store, error)) (bool, error) {

	// Check caravan-level dependency blocking.
	blocked, _, err := ss.IsWritBlockedByCaravanDeps(writID)
	if err != nil {
		return false, err
	}
	if blocked {
