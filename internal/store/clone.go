	...
	if includeHistory {
		...
		if _, err := tx.Exec(`
			INSERT INTO main.token_usage
				(id, history_id, model, input_tokens, output_tokens, cache_read_tokens, cache_creation_tokens, cost_usd, duration_ms, runtime, account, reasoning_tokens)
			SELECT id, history_id, model, input_tokens, output_tokens, cache_read_tokens, cache_creation_tokens, cost_usd, duration_ms, runtime, account, reasoning_tokens
