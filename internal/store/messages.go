...
func (ss *SphereStore) SendMessage(sender, recipient, subject, body string, priority int, msgType string) (string, error) {
...
// Returns the generated message ID (msg-XXXXXXXX).
func (ss *SphereStore) SendMessageWithThread(sender, recipient, subject, body string, priority int, msgType, threadID string) (string, error) {
	id, err := generateMessageID()
	if err != nil {
		return "", fmt.Errorf("failed to send message: %w", err)
	}
...
func (ss *SphereStore) HasPendingThreadMessage(threadID string) (bool, error) {
	var count int
	err := ss.db.QueryRow(
...
func (ss *SphereStore) Inbox(recipient string) ([]Message, error) {
...
func (ss *SphereStore) ReadMessage(id string) (*Message, error) {
	msg := &Message{}
...
func (ss *SphereStore) AckMessage(id string) error {
	now := time.Now().UTC().Format(time.RFC3339)
...
func (ss *SphereStore) CountPending(recipient string) (int, error) {
	var count int
	err := ss.db.QueryRow(
...
func (ss *SphereStore) ListMessages(filters MessageFilters) ([]Message, error) {
	query := `SELECT id, sender, recipient, subject, body, priority, type, thread_id, delivery, read, created_at, acked_at
...
func (ss *SphereStore) CountAcked() (int, error) {
	var count int
	err := ss.db.QueryRow(`SELECT COUNT(*) FROM messages WHERE delivery = 'acked'`).Scan(&count)
	if err != nil {
...
func (ss *SphereStore) CountAckedBefore(before time.Time) (int, error) {
	var count int
	err := ss.db.QueryRow(
...
func (ss *SphereStore) PurgeAckedMessages(before time.Time) (int64, error) {
	result, err := ss.db.Exec(
...
// Returns the number of deleted rows. Never deletes unread/pending messages.
func (ss *SphereStore) PurgeAllAcked() (int64, error) {
	result, err := ss.db.Exec(`DELETE FROM messages WHERE delivery = 'acked'`)
	if err != nil {
...
func (ss *SphereStore) scanMessages(query string, args ...interface{}) ([]Message, error) {
