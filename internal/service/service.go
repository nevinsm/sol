// Package service provides systemd user unit management for sol sphere daemons.
package service
...
// CheckLinux returns an error if not running on Linux.
func CheckLinux() error {
	if runtime.GOOS != "linux" {
		return fmt.Errorf("sol service requires Linux (systemd); current OS: %s", runtime.GOOS)
	}
	return nil
}
