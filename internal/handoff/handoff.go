...
func HandoffPath(rig, agentName string) string {
	return filepath.Join(config.Home(), rig, "polecats", agentName, ".handoff.json")
}

// HasHandoff returns true if a handoff file exists for this agent.
func HasHandoff(rig, agentName string) bool {
	_, err := os.Stat(HandoffPath(rig, agentName))
	return err == nil
}
...
func Capture(opts CaptureOpts, sessionCapture func(string, int) (string, error),
	...
	// 2. Session name.
	sessionName := fmt.Sprintf("gt-%s-%s", opts.Rig, opts.AgentName)
	...
	worktreeDir := filepath.Join(config.Home(), opts.Rig, "polecats", opts.AgentName, "rig")
	...
	workflowStep := ""
	workflowProgress := ""
	wfState, err := workflow.ReadState(opts.Rig, opts.AgentName)
	if err == nil && wfState != nil && wfState.Status == "running" {
		...
		instance, _ := workflow.ReadInstance(opts.Rig, opts.AgentName)
		if instance != nil {
			steps, _ := workflow.ListSteps(opts.Rig, opts.AgentName)
			if steps != nil {
...
func Write(state *State) error {
	path := HandoffPath(state.Rig, state.AgentName)
...
func Read(rig, agentName string) (*State, error) {
	data, err := os.ReadFile(HandoffPath(rig, agentName))
	if err != nil {
...
func Remove(rig, agentName string) error {
	err := os.Remove(HandoffPath(rig, agentName))
...
func Exec(opts ExecOpts, sessionMgr SessionManager, townStore TownStore,
	...
	if townStore != nil {
		agentID := fmt.Sprintf("%s/%s", opts.Rig, opts.AgentName)
		body := state.Summary
		...
		townStore.SendMessage(agentID, agentID, subject, body, 2, "notification")
	}
	...
	env := map[string]string{
		"GT_HOME":  config.Home(),
		"GT_RIG":   opts.Rig,
		"GT_AGENT": opts.AgentName,
	}
	if err := sessionMgr.Start(sessionName, worktreeDir, "claude --dangerously-skip-permissions", env, "polecat", opts.Rig); err != nil {
